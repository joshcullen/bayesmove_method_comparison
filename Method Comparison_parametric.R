...
# Load elapsed time
seg.time<- read.csv("Bayesian_elapsed_time_parametric.csv")
lda.time<- read.csv("LDA_elapsed_time_parametric.csv")
bcpa.time<- read.csv("BCPA_elapsed_time_parametric.csv")
hmm.time<- read.csv("HMM_elapsed_time_parametric.csv")
segclust.time<- read.csv("Segclust2d_elapsed_time_parametric.csv")
embc.time<- read.csv("EMbC_elapsed_time_parametric.csv")

# Load breakpoints
bayes.brkpts<- read.csv("Bayesian_allbreakpts_parametric.csv")
bcpa.brkpts<- read.csv("BCPA_allbrkpts_parametric.csv")
segclust.brkpts<- read.csv("Segclust2d allbrkpts_parametric.csv")

# Load results
bayes.res_parametric<- read.csv("Modeled MM Sim Tracks w Behav_parametric.csv")
hmm.res_parametric<- read.csv("HMM results_parametric.csv")
segclust.res_parametric<- read.csv("Segclust2d results_parametric.csv")
embc.res_parametric<- read.csv("EMbC results_parametric.csv")

# Load true breakpoints
true.brkpts_parametric<- read.csv("CRW_MM_sim_brkpts_parametric.csv")
...
bayes.res_parametric$method<- rep("Bayesian", nrow(bayes.res_parametric))
...
res_parametric<- rbind(bayes.res_parametric[,c("id","behav_fine","behav_coarse","track_length",
                                               "state", "method")],
                  hmm.res_parametric3[,c("id","behav_fine","behav_coarse","track_length","state",
                                         "method")],
                  segclust.res_parametric3[,c("id","behav_fine","behav_coarse","track_length",
                                              "state", "method")],
                  embc.res_parametric3[,c("id","behav_fine","behav_coarse","track_length","state",
                                     "method")])
...
# ggsave("Figure 3 (method comparison)_updatedParametric.png", width = 12, height = 12, units = "in", dpi = 330)
...
### Define bin limits
dat_parametric<- read.csv("CRW_MM_sim_parametric.csv", as.is = T)
...
hmm.SL.params_parametric<- read.csv("HMM result step params_parametric.csv", as.is = T)  
hmm.TA.params_parametric<- read.csv("HMM result angle params_parametric.csv", as.is = T)  
...
segclust.params_parametric<- read.csv("Segclust result state params_parametric.csv", as.is = T) %>%
  dplyr::select(-1)
...
samp.size<- embc.res_parametric %>%  #for use in merging Gaussians of HL and HH
  ...
  tally() %>% 
  drop_na()
embc.params_parametric<- read.csv("EMbC result state params_parametric.csv", as.is = T) %>% 
  dplyr::select(-1) %>% 
...
## Bayesian
behav.res_parametric<-  read.csv("CRW MM LDA Phi values_parametric.csv", as.is = T)
behav.res_parametric<- bayesmove::df_to_list(behav.res_parametric, "id")

behav.order_parametric<- read.csv("CRW MM LDA behavior order_parametric.csv", as.is = T)
...
for (i in 1:length(segclust.params2_parametric)) {
  segclust.b_parametric<- extract.behav.props_segclust(params = segclust.params2_parametric[[i]],
                                                  lims = list(dist.bin.lims, angle.bin.lims),
                                                  behav.names = c("Encamped","ARS","Transit"))
...
for (i in 1:length(embc.params2_parametric)) {
  embc.b_parametric<- extract.behav.props_segclust(params = embc.params2_parametric[[i]],
                                              lims = list(dist.bin.lims, angle.bin.lims),
                                              behav.names = c("Encamped","ARS","Transit"))
...
p.comp_hmm<- plot_grid(p.SL_hmm + theme(legend.position="none") + 
                         ggtitle("HMM", subtitle = "Standard distributions"),
                       NA,
...
p.comp_segclust<- plot_grid(p.SL_segclust + theme(legend.position="none") + 
                              ggtitle("Segclust2d", subtitle = "Standard distributions"),
                            NA,
...
p.comp_embc<- plot_grid(p.SL_embc + theme(legend.position="none") + 
                          ggtitle("EMbC", subtitle = "Standard distributions"),
                            NA,
