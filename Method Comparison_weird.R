...
segclust.time<- read.csv("Segclust2d_elapsed_time_weird.csv")
embc.time<- read.csv("EMbC_elapsed_time_weird.csv")
...
embc.res_weird<- read.csv("EMbC results_weird.csv")
...
segclust.params_weird<- read.csv("Segclust result state params_weird.csv", as.is = T) %>% 
  dplyr::select(-1)
...
samp.size<- embc.res_weird %>%  #for use in merging Gaussians of HL and HH
  ...
  tally() %>% 
  drop_na()
embc.params_weird<- read.csv("EMbC result state params_weird.csv", as.is = T) %>% 
  dplyr::select(-1) %>% 
