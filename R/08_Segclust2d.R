...
## Combine all model results
segclust.res<- c(segclust.res_1k, segclust.res_5k)
...
#save model params
# write.csv(segclust.state.params, "data/Segclust(HMM) result state params_weird.csv")
...
# Augment state estimates
segclust.aug2<- segclust.mod %>% 
  map(., augment, ncluster = 3, colname_state = "state.segclust") %>% 
  map(., 
      ~rbind(c(unique(.$id), 0, 0, rep(NA, 4), unique(.$track_length), rep(NA, 10)),
             .x)) %>% 
  bind_rows()
...
# write.csv(segclust.aug2, "data/Segclust2d(HMM) results_weird.csv", row.names = F)
# write.csv(time, "data/Segclust2d(HMM)_elapsed_time_weird.csv", row.names = F)  #units = min
...
#save model params
# write.csv(segclust.state.params, "data/Segclust(HMM) result state params_parametric.csv")
...
# write.csv(segclust.aug2, "data/Segclust2d(HMM) results_parametric.csv", row.names = F)
# write.csv(time, "data/Segclust2d(HMM)_elapsed_time_parametric.csv", row.names = F)  #units = min
