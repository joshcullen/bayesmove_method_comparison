...
################################################################
### Analyze HMM-based simulations with unusual distributions ###
################################################################

### Prep Data ###

d<- read.csv("data/HMM_sim_weird.csv", as.is = T)
names(d)[1]<- "ID"
d.list<- df_to_list(d, ind = "ID")

#format data
move.d<- map(d.list, ~prepData(.[,c(1,3:4)], type = "UTM", coordNames = c("x","y")))
...
## Combine all model results
hmm.res<- c(hmm.res_1k, hmm.res_5k)
...
#save model params
# write.csv(hmm.step.params, "data/HMM(HMM) result step params_weird.csv")
# write.csv(hmm.angle.params, "data/HMM(HMM) result angle params_weird.csv")
...
# AIC suggested all sims w/ 3 states
# BIC suggested all sims w/ 3 states
...
# write.csv(hmm.states, "data/HMM(HMM) results_weird.csv", row.names = F)
# write.csv(time, "data/HMM(HMM)_elapsed_time_weird.csv", row.names = F)  #units = min
...
################################################################
### Analyze HMM-based simulations with common distributions ###
################################################################

### Prep Data ###

d<- read.csv("data/HMM_sim.csv", as.is = T)
names(d)[1]<- "ID"
d.list<- df_to_list(d, ind = "ID")
...
#save model params
# write.csv(hmm.step.params, "data/HMM(HMM) result step params_parametric.csv")
# write.csv(hmm.angle.params, "data/HMM(HMM) result angle params_parametric.csv")
...
# write.csv(hmm.states, "data/HMM(HMM) results_parametric.csv", row.names = F)
# write.csv(time, "data/HMM(HMM)_elapsed_time_parametric.csv", row.names = F)  #units = min
