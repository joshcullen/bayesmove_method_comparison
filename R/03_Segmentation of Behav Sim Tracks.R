...
# track_length == 1000; takes 22 min for 40000 iterations
dat.res_1k<- segment_behavior(data = behav.list2[1:5], ngibbs = ngibbs, nbins = c(5,8),
                              alpha = alpha)

set.seed(1)
ngibbs = 80000
# track_length == 5000; takes 183 min for 80000 iterations
dat.res_5k<- segment_behavior(data = behav.list2[6:10], ngibbs = ngibbs, nbins = c(5,8),
                              alpha = alpha)
...
blank<- data.frame(matrix(NA, nrow = nrow(dat.res$nbrks), ncol = 40000))
names(blank)<- paste0("Iter_", 40001:80000)
...
MAP.est2<- get_MAP(dat.res$LML[6:10,], nburn = 80000/2)
MAP.est<- c(MAP.est, MAP.est2)
...
# track_length == 1000; takes 23 min for 40000 iterations
dat.res_1k<- segment_behavior(data = behav.list2[1:5], ngibbs = ngibbs, nbins = c(5,8),
                              alpha = alpha)

# set.seed(2021)
# ngibbs = 80000
# track_length == 5000; takes 137 min for 80000 iterations; takes 60 min for 40000 iter
dat.res_5k<- segment_behavior(data = behav.list2[6:10], ngibbs = ngibbs, nbins = c(5,8),
                              alpha = alpha)
...
##Determine maximum a posteriori (MAP) estimate for selecting breakpoints
MAP.est<- get_MAP(dat.res$LML, nburn = 40000/2)

brkpts<- get_breakpts(dat = dat.res$brkpts, MAP.est = MAP.est)



## Run segmentation model again on 5000 obs tracks and pre-specify MAP brkpts from last run

#create list of MAP brkpts
MAP.brks<- brkpts[6:10,] %>% 
  df_to_list("id") %>% 
  map(., ~{.x %>% 
      as.numeric() %>% 
      na.omit()})
...
plan(multisession, workers = 5)
#takes 60 min for 40000 iter
dat.res_5k_2<- segment_behavior(data = behav.list2[6:10], ngibbs = ngibbs, nbins = c(5,8),
                              alpha = alpha, breakpt = MAP.brks)

## Traceplots
#type is either 'nbrks' or 'LML' for y-axis label
traceplot(data = dat.res_5k_2$nbrks, ngibbs = ngibbs, type = "nbrks")
traceplot(data = dat.res_5k_2$LML, ngibbs = ngibbs, type = "LML")
